...
where ibd.bid_status = 'won' group by ibd.bidder_id, NO_OF_BIDS
...
from ipl_match_schedule ims join ipl_match im
join ipl_stadium s on ims.stadium_id = s.stadium_id
...
select Match_id, (select team_name from ipl_team it where if(im.match_winner = 1, it.team_id = im.team_id1, it.team_id = im.team_id2)) as 'Match Winner', 
 if(match_winner =1, team_id1, team_id2) as Winning_Team_ID,(select team_name from ipl_team it 
 where if(im.match_winner = 2, it.team_id = im.team_id1, it.team_id = im.team_id2)) as 'Match against', win_details
from ipl_match im;

## 6. Display total matches played, total matches won and total matches lost by team along with  its team name.
select its.TEAM_ID, TEAM_NAME, sum(MATCHES_PLAYED)as 'Matches Played', sum(MATCHES_WON)
as 'Matches Won', sum(MATCHES_LOST) as 'Matches Lost', sum(TOTAL_POINTS) as "Points"
...
from ipl_team_players itp join ipl_player ip
...
from ipl_team_players itp join ipl_player ip
on itp.player_id = ip.player_id join ipl_team it
